...
- [ ] `streamlit run app.py` chạy được, thấy trang chủ (chưa cần implement gì)
- [ ] `pytest -v` chạy được — tất cả test đang **fail** vì `NotImplementedError`,
...
- [ ] `pad_matrix(matrix, pad, mode="zero")` — thêm viền 0 quanh ma trận 2D
...
- [ ] `conv2d(input, kernel, stride=1, padding=0)` — 1 kênh, dùng
...
- [ ] `pytest tests/test_convolution.py::test_conv2d_stride` pass (stride>1)
...
- [ ] `conv_output_shape(input_size, kernel_size, stride, padding)` →
...
- [ ] `cnn_core/pooling.py`: `max_pool2d(input, size, stride)`,
...
- [ ] `cnn_core/activation.py`: `relu(x)`, `sigmoid(x)`
...
- [ ] `conv2d_multichannel(input_hwc, kernels, stride=1, padding=0)` trong
      `convolution.py` — mở rộng `conv2d` sang nhiều kênh input + nhiều
      kernel (mỗi kernel ra 1 feature map)
- [ ] `class ConvBlock`: `__init__(kernels, bias, pool_size, pool_stride)`,
...
- [ ] `pages/8_Full_Pipeline.py`: xếp 2-3 `ConvBlock` liên tiếp, upload/chọn
